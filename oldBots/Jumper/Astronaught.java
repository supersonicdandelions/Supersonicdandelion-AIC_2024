...
public class Astronaught {
    ...
    public void attackThenCollect(UnitController uc) {
        ...
        for (Direction dir : directions) {
            ...
            if (cp==CarePackage.PLANTS || cp==CarePackage.OXYGEN_TANK || cp==CarePackage.SURVIVAL_KIT || cp==CarePackage.REINFORCED_SUIT || cp==CarePackage.HYPERJUMP) {
        ...
        for (Direction dir : directions) {
            ...
            if (suited!=null && suited.getCarePackage()==CarePackage.REINFORCED_SUIT && suited.getTeam()==uc.getTeam() && (suited.getOxygen()<10)) {
    ...
    public CarePackageInfo[] toPickUp(CarePackageInfo[] seen) {
        ...
        for (CarePackageInfo carePackageInfo:seen) {
            if (carePackageInfo.getCarePackageType()==CarePackage.OXYGEN_TANK || carePackageInfo.getCarePackageType()==CarePackage.SURVIVAL_KIT || carePackageInfo.getCarePackageType()==CarePackage.PLANTS || carePackageInfo.getCarePackageType()==CarePackage.REINFORCED_SUIT || carePackageInfo.getCarePackageType()==CarePackage.HYPERJUMP) {
                length++;
            }
        }
    ...
    public void bugNav(UnitController uc,Location location) {
        ...
        while (canJump(uc,greedyDir) || canJump(uc,greedyDir.rotateRight()) || canJump(uc,greedyDir.rotateLeft().rotateLeft())) {
    ...
    public Location getTransfer(UnitController uc) {
        Location bestLoc=null;
        double lowOx=Double.POSITIVE_INFINITY;
        for (AstronautInfo suited: uc.senseAstronauts(8,uc.getTeam())) {
            if (suited.getCarePackage()==CarePackage.REINFORCED_SUIT && suited.getOxygen()<lowOx && distanceGraph[suited.getLocation().x-uc.getLocation().x+6][suited.getLocation().y-uc.getLocation().y+6]<2.9) {
                if (suited.getOxygen()<10) {
                    bestLoc=suited.getLocation();
                    lowOx=suited.getOxygen();
                }
            }
        }
        return bestLoc;
