                ...
                for (Direction dir : directions) {
                    if (uc.canEnlistAstronaut(dir, spawnValue, null)) {
                        if (uc.getStructureInfo().getCarePackagesOfType(CarePackage.HYPERJUMP) > 0) {
                            uc.enlistAstronaut(dir,(int)GameConstants.MIN_OXYGEN_ASTRONAUT,CarePackage.HYPERJUMP);
                        }
                        else if (uc.getStructureInfo().getCarePackagesOfType(CarePackage.SURVIVAL_KIT) > 0) {
