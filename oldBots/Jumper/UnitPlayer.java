    ...
    public void run(UnitController uc) {
        // Code to be executed only at the beginning of the unit's lifespan
            ...
            //Case in which we are an astronaut
                ...
                if (uc.getAstronautInfo().getCarePackage()==CarePackage.HYPERJUMP) {
                    new Jumper(uc);
                }
                else if (uc.getAstronautInfo().getCarePackage() == CarePackage.REINFORCED_SUIT || uc.getAstronautInfo().getOxygen()== (int)GameConstants.MIN_OXYGEN_ASTRONAUT) {
                    new Drone(uc);
                }
                else {
                    new Astronaught(uc);
