...
public class Astronaught {
    ...
    public Location graphClosest(UnitController uc,AstronautInfo[] astronautInfos) {
        ...
        for (AstronautInfo astronautInfo:astronautInfos) {
            ...
            if (value<distance) {
                ...
                bestOx=astronautInfo.getOxygen();
            }
            if (value==distance) {
                if (astronautInfo.getOxygen()>bestOx) {
                    bestLoc=astronautInfo.getLocation();
                    bestOx=astronautInfo.getOxygen();
                }
            }
    ...
    public AstronautInfo findNearestMin(Location loc, AstronautInfo[] astronautInfos) {
        AstronautInfo toReturn=null;
        double bestOx=Double.POSITIVE_INFINITY;
        double distance=Double.POSITIVE_INFINITY;
        for (AstronautInfo astronautInfo:astronautInfos) {
            double value=loc.distanceSquared(astronautInfo.getLocation());
            if (value<distance) {
                distance=value;
                toReturn=astronautInfo;
                bestOx=astronautInfo.getOxygen();
            }
            if (value==distance) {
                if (astronautInfo.getOxygen()<bestOx) {
    ...
    public void bugNavMicroMove(UnitController uc) {
        AstronautInfo fleeFrom=findNearestMin(uc.getLocation().add(dir),flee);
        if (fleeFrom!=null && uc.getLocation().add(dir).distanceSquared(fleeFrom.getLocation())<9 && distanceGraph[fleeFrom.getLocation().x-uc.getLocation().x+6][fleeFrom.getLocation().y-uc.getLocation().y+6]<4.3) {
            Direction fleeDir = uc.getLocation().directionTo(fleeFrom.getLocation()).opposite();
            if (uc.canPerformAction(ActionType.MOVE, fleeDir, 0)) {
                uc.performAction(ActionType.MOVE, fleeDir, 0);
                bugNaving = false;
                uc.println("fleeing");
            } else {
                fleeDir = fleeDir.rotateRight();
                if (uc.canPerformAction(ActionType.MOVE, fleeDir, 0)) {
                    uc.performAction(ActionType.MOVE, fleeDir, 0);
                    bugNaving = false;
                    uc.println("fleeing");
                } else {
                    fleeDir = fleeDir.rotateLeft().rotateLeft();
                    if (uc.canPerformAction(ActionType.MOVE, fleeDir, 0)) {
                        uc.performAction(ActionType.MOVE, fleeDir, 0);
                        bugNaving = false;
                        uc.println("fleeing");
    ...
    public void moveWithMicro(UnitController uc,Location location) {
        Direction dir=nextGraphMove(uc,location);
        if (dir!=null) {
            if (uc.canPerformAction(ActionType.MOVE,dir,0)) {
                if (distanceGraph[location.x-uc.getLocation().x+6][location.y-uc.getLocation().y+6] < 2.9) {
                    uc.performAction(ActionType.MOVE, dir, 0);
                    bugNaving = false;
                ...
                AstronautInfo fleeFrom = findNearestMin(uc.getLocation().add(dir), flee);
                if (fleeFrom != null && distanceGraph[fleeFrom.getLocation().x-uc.getLocation().x+6][fleeFrom.getLocation().y-uc.getLocation().y+6] < 2.9) {
                    Direction fleeDir = uc.getLocation().directionTo(fleeFrom.getLocation()).opposite();
                    if (uc.canPerformAction(ActionType.MOVE, fleeDir, 0)) {
                        uc.performAction(ActionType.MOVE, fleeDir, 0);
                        bugNaving = false;
    ...
    public void getAttackAndFlee(UnitController uc) {
        ...
        for (AstronautInfo enemyAstro:potentials) {
            if (enemyAstro.getOxygen()>=myOx) {
                attack[sizeA]=enemyAstro;
                sizeA++;
            }
            else {
                flee[sizeF]=enemyAstro;
                sizeF++;
            }
        }
    }
